var firtTime = true;
      ...
      const myTimeout = setTimeout(upSpped, 5000);
      //   speedUpFunction
      function upSpped() {
        player.speed = player.speed + 1;
        console.log("speed", player.speed);
        setTimeout(upSpped, 5000);
      }
      ...
      function moveEnemy(myCar) {
        ...
        let myyCar = document.querySelectorAll(".myCar");
        if (enemyCarList[0].y > myyCar[0].offsetTop) {
          if (firtTime) {
            player.score += 10;
            firtTime = false;
            ThirdTime = true;
          }
        } else if (enemyCarList[1].y > myyCar[0].offsetTop) {
          if (SecondTime) {
            player.score += 10;
            SecondTime = false;
            firtTime = true;
          }
        } else if (enemyCarList[2].y > myyCar[0].offsetTop) {
          if (ThirdTime) {
            player.score += 10;
            ThirdTime = false;
            SecondTime = true;
